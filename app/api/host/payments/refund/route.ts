...
const refundSchema = z.object({
  host_payment_id: z.string().uuid(),
  refund_amount: z.number().positive().optional(),
  refund_note: z.string().max(200).optional(),
});

/**
 * Refund a host PPA payment via Cashfree. Admin-only.
 */
export async function POST(request: Request) {
  try {
    ...
    const { host_payment_id, refund_amount, refund_note } = parsed.data;

    const { data: payment } = await admin
      .from("host_payments")
      .select("id, total_amount, status, gateway_order_id, gateway_payment_id, invoice_id")
      .eq("id", host_payment_id)
      .maybeSingle();

    if (!payment) {
      return NextResponse.json(
        { success: false, error: { code: "PAYMENT_NOT_FOUND", message: "Host payment not found" } },
        { status: 404 }
      );
    }
    ...
    const amount = refund_amount ?? Number(payment.total_amount);
    const refundId = `rfnd_host_${payment.id}_${Date.now()}`;
    ...
    console.error("Host refund error:", error);
    return NextResponse.json(
